...
const main = () => {
  const feed = new RSS({
    ...
    site_url: "https://houssaineamzil.vercel.app",
    feed_url: "https://houssaineamzil.vercel.app/feed.xml",
    image_url: "https://houssaineamzil.vercel.app/og.png",
    language: "en",
  })

  posts.forEach((post) => {
    const url = `https://houssaineamzil.vercel.app/blog/${post.slug}`

    feed.item({
      title: post.title,
      description: renderPost(post.body),
      date: new Date(post.date),
      author: "Høussaine Amzil",
      url,
      guid: url,
    })
  })
