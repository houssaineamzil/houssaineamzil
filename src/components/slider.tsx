  ...
  const horizontalLoop = useCallback(
    (
      ...
      function toIndex(index: number, vars: gsap.TweenVars = {}) {
        ...
        if (Math.abs(updatedIndex - curIndex) > length / 2)
          updatedIndex += updatedIndex > curIndex ? -length : length; // always go in the shortest direction
        const newIndex = gsap.utils.wrap(0, length, updatedIndex);
        let time = times[newIndex];
        if (time > timeline.time() !== updatedIndex > curIndex) {
          // if we're wrapping the timeline's playhead, make the proper adjustments
          vars.modifiers = { time: gsap.utils.wrap(0, timeline.duration()) };
          time += timeline.duration() * (updatedIndex > curIndex ? 1 : -1);
        }
        curIndex = newIndex;
        vars.overwrite = true;
